...
class InvertibleConv1x1(nn.Module):
    def __init__(self, num_channels, LU_decomposed):
        ...
        w_init = torch.qr(torch.randn(*w_shape))[0] # QR decomposition of a matrix , 为了使w_init行列式初始值不为0

        if not LU_decomposed:
            self.weight = nn.Parameter(torch.Tensor(w_init))
        else:
            p, lower, upper = torch.lu_unpack(*torch.lu(w_init)) # Unpacks the LU decomposition into the P, L, U matrices
            s = torch.diag(upper)
            sign_s = torch.sign(s)
            log_s = torch.log(torch.abs(s))
            upper = torch.triu(upper, 1)                  #Returns the upper triangular part of a matrix
            l_mask = torch.tril(torch.ones(w_shape), -1)   #Returns the lower triangular part of the matrix 
            eye = torch.eye(*w_shape)                      #Returns a 2-D tensor with ones on the diagonal and zeros elsewhere.
    ...
    def get_weight(self, input, reverse):
        ...
        if not self.LU_decomposed:
            ...
            if reverse:
                weight = torch.inverse(self.weight) # Computes the inverse of a square matrix if it exists
            else:
                weight = self.weight
        else:
