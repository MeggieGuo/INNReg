...
class REGISTRATIONModel(BaseModel):
    ...
    def forward(self):
        ...
        self.fake = self.netG(self.realAA)  # output fake_B=T(x), idt_B=T(y)
        self.idt_B = self.fake[:,:self.real_A.size(1)] #torch.Size([2, 1, 256, 256])
        ...
        self.fake_rev = self.netG(self.realBB, rev=True)  # output fake_B=T(x), idt_B=T(y)
        self.idt_A = self.fake_rev[:,:self.real_A.size(1)] #torch.Size([2, 1, 256, 256])
