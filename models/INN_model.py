...
from .DW_model import DWBlock, DWDBNet
...
def subnet(net_structure, init='xavier'):
    def constructor(channel_in, channel_out):
        ...
        elif net_structure == 'DWBlock':
            return DWBlock(128, 7, dynamic=True, inhomogeneous=False)
        elif net_structure == 'DWDB':
            return DWDBNet(channel_in, channel_out, 128, 7, init)
        else:
            return None
    return constructor
...
class InvBlock(nn.Module):
    def __init__(self, subnet_constructor, channel_num, channel_split_num, clamp=0.8):
        ...
        self.split_len2 = channel_split_num # 1
    ...
    def forward(self, x, rev=False):
        if not rev:            
            ...
            # print(x1.shape, x2.shape, self.F(x2).shape)
            # exit()
...
class InvISPNet(nn.Module):
    def __init__(self, channel_in=3, channel_out=3, subnet_name = 'DBNet', block_num=8): #subnet_constructor=subnet('DBNet'), 
        ...
        for j in range(block_num): 
            ...
            b = InvBlock(subnet(subnet_name), channel_num, channel_split_num) # one block is one flow step. 
            operations.append(b)
    ...
    def forward(self, x, rev=False):
        out = x # x: [N,3,H,W] 
        # out = x.repeat(1, 3, 1, 1) # x: [N,3,H,W] 
